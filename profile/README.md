# AppTeam Repo's

| Repository | Short Description |
| --- | --- |
| [`.github`](https://github.com/FSW-AppTeam/.github) | Organization profile and shared GitHub metadata. |
| [`FSW-orsee`](https://github.com/FSW-AppTeam/FSW-orsee) | Online Recruitment System for Economic Experiments |
| [`FSW-VINA`](https://github.com/FSW-AppTeam/FSW-VINA) | VINA application maintained by the FSW AppTeam. |

# FSW-LimeSurvey repo's

| Repository | Short Description |
| --- | --- |
| [`FSW-LimeSurvey-develop`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-develop) | Development environment for local development + Documentation |
| [`FSW-LimeSurvey`](https://github.com/FSW-AppTeam/FSW-LimeSurvey) | Fork of the original LimeSurvey repo. |
| [`FSW-LimeSurvey-AppTeam-plugin`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-AppTeam-plugin) | AppTeam plugin to various custom features |
| [`FSW-LimeSurvey-AuditLogFile-plugin`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-AuditLogFile-plugin) | Plugin to write the records from the Auditlog plugin to a log file |
| [`FSW-LimeSurvey-SurfConext-plugin`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-SurfConext-plugin) | A LimeSurvey plugin for connection to SurfConext OIDC |
| [`LimeSurveyOIDCAuth`](https://github.com/FSW-AppTeam/LimeSurveyOIDCAuth) | Deprecated: An OIDC authentication plugin for LimeSurvey 6.x. |
| [`LimeSurvey-UU-theme`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-UU-theme) | UU theme for the survey's and for the survey overview page |
| [`LimeSurvey-UU-Fruity-theme`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-UU-Fruity-theme) | UU Fruitye theme for the survey's and for the survey overview page, required by new LimeSurvey 7 editor|
| [`LimeSurvey-UU-Admin-theme`](https://github.com/FSW-AppTeam/FSW-LimeSurvey-UU-Admin-theme) | UU theme for the amdmin inlog page |
